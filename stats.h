...
float track_stat(node* list, int bracketType, int bracket, int category,
		int from, int to) {
// from and to are the choices in the survey, they vary based on category
	...
	if (surveysInAgeBracket == 0) {
		totalPercent = 0; // prevent division by 0
	} else {
		totalPercent = (inThisCategory * 100) / surveysInAgeBracket;
...
void print_stats(node* list, FILE* file) {
	...
	char ageBrackets[5][10] = { { "18 - 20" }, { "20 - 30" }, { "30 - 50" }, {
			"50 - 65" }, { "65 + " } };
	char incBrackets[7][25] = { { "No Income" }, { "Less Than €20,000" }, {
			"Less Than €40,000" }, { "Less Than €60,000" }, {
			"Less Than €80,000" }, { "Less Than €100,000" }, {
			"More Than €100,000" } };
	int i;
	...
	for (i = 1; i <= 5; i++) { // 5 age brackets
		...
		fprintf(file,
				"People Who Exercise Less Than Three Times Per Week: %.2f%c\n",
				track_stat(list, AGE, i, EXERCISE, 1, 2), '%');
		// options 1 and 2 are < 3 times
		fprintf(file,
				"People Who Exercise Less Than Five Times Per Week: %.2f%c\n",
				track_stat(list, AGE, i, EXERCISE, 1, 3), '%');
		// options 1,2 and 3
		fprintf(file,
				"People Who Exercise More Than Five Times Per Week: %.2f%c\n",
